...
class Command(BaseCommand):
    args = '<set set ...>'
    help = 'Updates the specified sets'

    # A shortcut for printing stuff if the verbosity is high enough
    def p(self, string, flush=False):
        if self.verbosity:
            self.stdout.write(string)
            if flush:
                self.stdout.flush()

    # Switches from a list to a dict and does some rudimentary normalization
    ...
    # Converts a mana cost string to a list of string tokens
    ...
    # Converts a tokenized mana cost into it's converted value
    ...
    # Converts a tokenized mana cost into it's string representation
    ...
    # Extracts a card's card types from its type line
    ...
    # Extracts a card's sub types from its type line
    ...
    # Makes sense of the "miscellaneous" field (power, toughness, etc.)
    ...
    # Splits a comma-separated string into a list of Set/Rarity objects
    ...
    def handle(self, *args, **kwargs):
        ...
        self.verbosity = int(kwargs['verbosity']) > 0
        ...
        self.p('converted_mana_cost, mana_cost...\n')
        for card in cards:
        ...
        # Determine types
        self.p('super_types, card_types, sub_types...\n')
        for card in cards:
        ...
        self.p('Associating many-to-many fields...\n')
        for card in cards:
